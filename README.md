...
- **证书安装失败**：以管理员身份重新运行程序，或手动将证书导入系统受信任的根证书颁发机构。
- **下载按钮不出现**：确保代理服务已启动，并且微信网络走系统代理（可重启微信后重试）。
- **视频下载后无法播放**：推荐使用 VLC 播放器，兼容性最好。
- **Windows 防火墙弹窗**：首次运行时若出现防火墙提示，选择「允许访问」，否则代理服务无法正常拦截请求。
- **macOS 证书信任问题**：安装证书后需在「钥匙串访问」中手动将证书设为「始终信任」，否则代理仍可能报证书错误。
- **下载目录在哪里**：默认下载到程序同级目录下的 `downloads` 文件夹中，可直接在文件管理器中查找。
